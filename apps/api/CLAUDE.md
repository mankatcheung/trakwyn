...
- **Tool-call observation (JEF-365):** `McpController.callTool` and `chatAssembly.executeChatTool` run every call through `IToolCallObserver.observe(meta, run)` (`use-cases/ports/`). The surface reports how the call ended through the recorder it is handed: `succeeded`, `failed`, `invalidParams` or `refused(userId)`. The OTel implementation is `infrastructure/observability/ToolCallObserver.ts`, registered in `http/di/infrastructure.ts` with the whole catalogue. A new dispatch path must go through `observe`; the parity tests in `McpController.test.ts` and `chatAssembly.test.ts` check every catalogue tool does.
...
- **Metrics:** counters live in `infrastructure/observability/metrics.ts` behind `IMetrics` and are created lazily (a meter obtained before `startObservability()` is a permanent no-op). Test them with `makeFakeMetrics()` (`__tests__/helpers/fakeMetrics.ts`). They cover:
  ...
  - `trakwyn.tool.calls` by `surface`/`tool`/`outcome`, and `trakwyn.mcp.tool_refused` by `tool`/`scope` (JEF-365).
- **MCP refusals:** a write tool called with a `read` token logs `mcp.tool.refused` at `warn` with `userId`, `tool` and `scope` (`SECURITY_EVENTS.MCP_TOOL_REFUSED`).
