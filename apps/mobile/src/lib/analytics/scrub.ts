 ...
 *
 * Kept in step with apps/web/src/lib/analytics/scrub.ts, which is the same
 * rule for the other SDK — mirrored rather than shared, following the
 ...
 *
 * `__tests__/scrubParity.test.ts` on each side asserts the two behave
 * identically on the same inputs, so the copies cannot quietly drift.
 */
