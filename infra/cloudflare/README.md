...
The DNS records and zone settings for `trakwyn.com`, as Terraform (JEF-371). Cloudflare is the zone's DNS host (nameservers `david` and `crystal.ns.cloudflare.com`). Before this, records were added by hand in the dashboard, including the ones `infra/vercel` and `infra/gcp` depend on, and nothing recorded what was supposed to be there.
...
Once the plan shows only changes you mean, `apply`, then empty `record_import_ids`. Import blocks are no-ops once the resource is in state, but the map is only needed once.
