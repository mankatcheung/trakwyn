...
| Web app server error            | Production has no safe way to make a render fail on demand, so post one line with the web ingest token: `curl -X POST https://eu-central-1.aws.edge.axiom.co/v1/ingest/trakwyn-web -H "Authorization: Bearer $AXIOM_WEB_TOKEN" -H 'Content-Type: application/json' -d '[{"event":"web.ssr.failed","phase":"test"}]'`. |
